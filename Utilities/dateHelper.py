...
def date_helper(date, return_value):
    ...
    elif return_value == 'year':
        return (matching_row['Year'].values)
    ...
    elif return_value == 'month_id':
        return (matching_row['Month ID'].values)
...
def rolling_13_month_list(date):
    date  = np.datetime64(date)
    ...
    # find out number of periods from prior year
    periods_from_prior_yr = 12 - prior_period
    ...
    for i in range(12, (12 - periods_from_prior_yr), -1):
        period_id = str(prior_year) + str(i).zfill(2)
        list_prior_yr.append(period_id)
